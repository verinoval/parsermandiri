...
class Mandiri {
	...
	*/
	...
	public function getArrayValues($html)
    {
        $dom = new DOMDocument();
        $dom->loadHTML($html);
        ...
        for ($i = 0; $i < $rows->length; $i++){
            ...
            $description = trim($cols->item(1)->nodeValue);
            $credit = trim($cols->item(3)->nodeValue);
            ...
            if($flows = 'CR'){
                $point_amount       = trim($cols->item(3)->nodeValue);
                $amount         = str_replace(',00','', $point_amount);
            }else{
                $point_amount      = trim($cols->item(2)->nodeValue);
                $amount         = str_replace(',00', '', $point_amount);
            }

            $data = compact('date','description','amount','flows');
            $datas[] = $data;
        }
        return $datas;
    }
